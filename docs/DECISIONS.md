...
## D-044 — Unity 6 LTS version pinned to 6000.3.22f1; runtime lane remains blocked pending two interactive environment actions

**Status:** Accepted *(version decision resolved by M5-R1 campaign evidence; implementation remains UNVERIFIED — no Editor ran)*

**Decision:** The Unity 6 LTS baseline for all presentation/runtime work is **6000.3.22f1** (changeset `1c726e1fb402`, released 2026-08-13, supported until December 2027). The 6000.0.x LTS line is rejected for new work because its support window closes October 2026. Android Build Support for that exact changeset is the only additional module authorized so far.

**Rationale:** This resolves open decision #1 recorded under D-023 ("Exact Unity 6 LTS editor/runtime version"). Choosing the newest LTS line avoids landing the project on a stream entering end-of-support within weeks of first use. The M5-R1 campaign proved every non-interactive installation path fails on a non-elevated Windows token (direct installer requires elevation by manifest; the installer is no longer NSIS-extractable; Unity Hub headless downloads succeed but its internal install step launches the same elevated installer; no portable archive is published) — see `docs/evidence/m5-r1/UNITY_ENVIRONMENT.md` for exact commands and outputs.

**Consequences:**
- Runtime gates stay UNVERIFIED and honestly labeled; the M5-R1 campaign closed as BLOCKED — HUMAN UNITY ENVIRONMENT ACTION REQUIRED (elevation approval + Personal-license sign-in), with an exact resume procedure.
- Plain authored Unity-side sources (`unity/Assets/WalkGame/**`) may be committed only when labeled UNVERIFIED and containing zero fabricated Editor-owned artifacts (.meta/scenes/ProjectSettings); all such files are generated by the real Editor during resume.
- Core consumption model for the shell: precompiled netstandard2.1 plugins staged from the existing solution by `scripts/build-unity-plugins.ps1`; System.Text.Json dependency closure ships alongside; `link.xml` preserves the core assemblies under IL2CPP.
