# Repository-Local Add-ons Master Plan — simple-walk-game

## Status

**PLANNING ONLY / IMPLEMENT IN A LATER SESSION**

Target repository: `quantdale/simple-walk-game`  
Planning branch: `plan/repo-local-addons-2026-08-28`

## Repository assessment

Unity 6 mobile ambient fitness game with deterministic offline-first domain architecture and planned Health/activity adapters; device/editor evidence is a key future gate.

**Decision:** `RECOMMEND_UNITY_EDITOR_PLUS_MOBILE_DIAGNOSTICS`

Raw health/activity data should never be exposed to agent tooling; use synthetic fixtures for automation.
...
**Why it fits:** Useful for scene/project setup, play-mode checks, screenshots, editor test execution and asset inspection without relying on manual editor interaction.

**Constraints:** Reverify the maintained upstream, pin repository-locally, editor-only/loopback-only, and keep a reduced tool profile.

### 2. Mobile MCP — RECOMMEND

**Type:** MCP

**Why it fits:** Useful later for emulator/device lifecycle and activity-adapter UI diagnostics.

**Constraints:** Dedicated test device only; verify patched current release because Mobile MCP has had security advisories. Never treat it as HealthKit/Health Connect truth or device-certification authority.

## Explicitly not recommended

- Global Unity packages/tools
- Broad health-data connectors
- MCP access to real personal health/activity data
