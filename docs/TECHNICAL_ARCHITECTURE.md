...
## M5-H1 addendum — platform-neutral UX state contracts (implemented and automated verified)

**Local UX-preferences store (D-042).** A second durable record exists beside the canonical save: versioned schema v1 JSON in a dedicated file behind the `IUxPreferencesStore` application port (`LocalPreferencesStore` implementation). It owns onboarding progress, reduced-motion/haptics/sound flags, notification opt-in + category toggles + optional reminder time-of-day, and the diagnostics-visibility flag. Write path is the same durable-temp-then-atomic-replace pattern as the save store but WITHOUT backup generations — proportionate to data value; worst case is preference reset, never gameplay loss. Load policy degrades explicitly (NotFound/Malformed/FutureVersion to documented defaults; v1 keys merge over defaults). This store never contains canonical game state and canonical saves are provably byte-neutral under preference churn.

**Activity connection status seam (D-043).** Ephemeral platform truth enters through one new port, `IActivityConnectionPort`, mirroring the D-034 adapter discipline for records: adapters report permission state (including partially-granted/revoked), availability, refresh timestamps, and one bounded technical-detail string. The player-safe standing classification is a pure projection (`ActivityStatusProjector`) combining that snapshot with two canonical facts (processed-record existence; durable last-ingestion outcome). Failed source fetches durably persist bounded outcome evidence (exception TYPE category only) in the same atomic discipline as successful batches, then rethrow. No native SDKs, OS dialogs, or device claims exist at this layer; future Health Connect/HealthKit adapters implement the same port and inherit conformance tables.

**Shell-facing projections.** New purpose-built read models follow the existing immutable-read-model rule (TECH section 15): `OnboardingReadModel`, `SettingsReadModel` (local preferences kept separate from canonical queue auto-advance), `ActivityStatusReadModel`, `DiagnosticsReadModel` (D-044 privacy-safe support facts only), plus explicit attention semantics on `HomeReadModel`. Presentation consumes these; it never reads domain graphs or preference internals.

**Persistence boundaries summary for M5-H1:** canonical save envelope untouched (schema stays v2; the additive `IngestionOutcomeState? LastIngestionOutcome` field decodes absent-as-null exactly like `PendingReturnSummary`); preferences live in their own versioned envelope; status is never persisted by the application; diagnostics derive from existing canonical aggregates plus the single bounded outcome row.
